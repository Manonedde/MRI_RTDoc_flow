...
def generate_individual_image_plot(data, map_prefix, slices_order,
    ...
    for slice_z in slices_order:
        ...
        image=ax.imshow(ndimage.rotate(data[:,:,slice_z],-90),
                        cmap = colmap, origin ='lower') #, interpolation='spline16')
        ax.axis('off')
        ax.set_facecolor('k')
...
def custom_colormap(colors, range=200):
...
def main():
    ...
    if args.in_png:
        ...
        if args.out_prefix is None:
            args.out_prefix = (os.path.splitext(os.path.basename(png_list[0]))[0])
