...
def main():
    ...
    for curr_json in args.in_jsons:
        filename = os.path.splitext(os.path.basename(curr_json))[0]
        tmp_json = pd.read_json(curr_json, orient='index').reset_index()
        tmp_json.columns = ['parameter','value']
        tmp_json['subject'] = filename

        for curr_param in args.parameters:
            if curr_param not in tmp_json['parameter'].unique().tolist():
                print('Parameter {} not found in json {}'.format(curr_param, filename))
            else:
                param_value = tmp_json[tmp_json['parameter'] == curr_param]['value']
